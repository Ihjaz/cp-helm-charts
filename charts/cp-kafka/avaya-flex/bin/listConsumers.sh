...
kubectl exec -it -c cp-kafka-broker eventing-kafka-cp-kafka-0 -- kafka-consumer-groups.sh --describe --group $3 --bootstrap-server eventing-kafka-cp-kafka-headless:9092
fi
